...
var FeedSchema = new Schema({
    uuid: { type: String, default: uuid.v4() }, // this is what is served, not the _id
    owner: String, // the owner's _id
    created: { type : Date, default: Date.now },
    updated: { type : Date, default: Date.now },
    content: String, // the content object's _id
    ...
    children: [ChildFeedSchema] // linked-list of child feeds
});
...
var findFeed = function(id, fn) {
    Feed.findById(id, function(err, f) {
        if (!err || f) fn(null, f);
